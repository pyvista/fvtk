...
VTK and cvista for every scene. The ids are integer values (width-relaxed): cvista's
source vtkOriginalPointIds array is int32, stock's is int64, but the selected
VALUES must match exactly. A mismatch means cvista's mapper failed to remap picked
...
Also enforces:
  ...
  * (informational) the cvista source array really is int32 -- proving the
    discriminating condition holds; a warning, not a failure, so the gate keeps
    working if the storage policy ever changes.

Usage:  python compare_select.py <stock_dir> <cvista_dir>   (exit 1 on any diff)
"""
...
def main():
    stock_dir, cvista_dir = sys.argv[1], sys.argv[2]
    sm = _load(stock_dir)
    fm = _load(cvista_dir)
    ...
    for name in scenes:
        ...
        if "error" in sc or "error" in fc:
            print(f"FAIL {name}: backend error stock={sc.get('error')} cvista={fc.get('error')}")
            n_fail += 1
            continue
        s_ids = np.load(os.path.join(stock_dir, name + ".npz"))["selected_point_ids"]
        f_ids = np.load(os.path.join(cvista_dir, name + ".npz"))["selected_point_ids"]
        if s_ids.size == 0:
        ...
        if s_ids.shape != f_ids.shape or not np.array_equal(
            ...
            print(
                ...
                f"(stock n={s_ids.size}, cvista n={f_ids.size}; "
                f"only-stock={only_s[:8].tolist()}, only-cvista={only_f[:8].tolist()})"
            )
            n_fail += 1
            continue
        # Informational: confirm the discriminating int32 storage is in effect.
        fdt = fc.get("orig_pointids_dtype")
        note = "" if fdt == "int32" else f"  [warn: cvista dtype={fdt}, expected int32]"
        print(f"OK   {name}: {s_ids.size} selected ids match "
              f"(stock dtype={sc.get('orig_pointids_dtype')}, cvista dtype={fdt}){note}")
