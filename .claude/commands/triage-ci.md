---
description: Diagnose a failing cvista CI run or PR check — fetch the failing job logs, classify the failure against the known gates, and propose or apply the fix.
...
- **bitexact** — output diverged from stock VTK 9.6.2 (`maxULP > 0`). This is a **parity
  ...
  again, move it behind `cvista.EnableFast()` as a Bucket-2 opt-in, or — if it is an intentional
...
- **regression** — an cvista regression scene failed. Read the specific test; it usually points at
  a concrete filter or render behavior.

- **pyvista** (PR #112, merge-queue / `pyvista-full` label) — PyVista's own suite failed against
  the built wheel. Split image-cache drift from a real cvista regression exactly as the
  `/pyvista-bump` command describes. Real regression → fix in cvista; drift → `deselect.txt` with a
...
- **sdk / sdk-gate** — the `cvista-sdk` build or its external-project consumer test failed (the SDK
...
- **publish** (release only) — runs only on a published GitHub Release; a failure here is
  trusted-publisher / environment config, not code. See `/cvista-release`.

## Reproduce, then fix

Reproduce locally before changing anything: `PROFILE=fast ./build-cvista.sh` plus the matching
