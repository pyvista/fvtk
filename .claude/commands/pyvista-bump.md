---
description: Advance the pinned PyVista ref and triage the parity suite — separate real cvista regressions (fix them) from image-cache drift (deselect them).
argument-hint: "[pyvista-sha-or-tag | latest]"
allowed-tools: Read, Edit, Bash, Grep, Glob
---

Bump the PyVista pin cvista tests against, run the parity suite, and triage the result. The suite
...
- `ci/run-pyvista.sh` — clones PyVista at the pin, installs it `--no-deps` over the built cvista
  wheel with the `vtkmodules` → `cvista` redirect, runs core + plotting offscreen under Xvfb +
...
4. Run the suite against a built cvista wheel: `ci/run-pyvista.sh <wheel-dir>` locally if a wheel
...
5. **Triage every failure. This is the real work — distinguish two kinds:**
   ...
   - **Real cvista regression** — a parity break, a crash, a wrong numerical result, an import
     that should have redirected but did not. **Do not deselect these.** Reproduce against the
     bitexact / renderexact gate, find the cvista cause, and fix it (or file it and leave the pin
     unbumped). A real regression means cvista diverged from the contract, which is the whole point
     of the gate.
...
7. Open the bump PR: title `test(pyvista): bump pin to <short-sha>`, body stating the old → new
   ref, the compare link, what was deselected and why, and any cvista fix that rode along. Follow
