#!/usr/bin/env bash
#
# Smoke-test the built cvista-sdk wheel: install it and confirm a downstream
...
"$PYBIN/python" -m pip install --no-index --find-links "$(cmpath "$OUT")" cvista-sdk
...
# depends on cvista-sdk; run it from the wheel dir so its `--find-links .` resolves
# the freshly built cvista-sdk wheel.
cd "$OUT"
...
# vtk_module_build + vtk_module_wrap_python against the same cvista-sdk wheel, then
...
# Same wheel dir so its `--find-links .` resolves the freshly built cvista-sdk wheel.
