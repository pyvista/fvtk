...
in the "cvista" package (or whichever package you specify).
...
Options are as follows:
 ...
 -p PACKAGE    The package to generate .pyi files for [cvista]
...
are put inside the existing cvista package).  This is equivalent
to the following:

    python -m cvista.generate_pyi -p cvista

To put the pyi files somewhere else, perhaps with a different suffix:

    python -m cvista.generate_pyi -o /path/to/cvista -e .pyi

To generate pyi files for just one or two modules:

    python -m cvista.generate_pyi -p cvista vtkCommonCore vtkCommonDataModel

To generate pyi files for your own modules in your own package:

    python -m cvista.generate_pyi -p mypackage mymodule [mymodule2 ...]

"""

from cvista.vtkCommonCore import vtkObjectBase, vtkSOADataArrayTemplate
...
def isnamespace(m):
    ...
    # until cvista.vtkCommonCore.namespace is directly accessible
    return (str(type(m)) == "<class 'cvista.vtkCommonCore.namespace'>")
...
def namespace_pyi(c, mod):
    ...
    """
    base = "namespace"
    if mod.__name__ != 'cvista.vtkCommonCore':
        base = 'cvista.vtkCommonCore.' + base
...
def module_pyi(mod, output):
    ...
    depends = set(['cvista.vtkCommonCore'])
...
def main(argv=sys.argv):
    ...
    parser.add_argument('-p', '--package', type=str, default="cvista",
                        help="Package name [cvista].")
