...
in the "fvtk" package (or whichever package you specify).
...
Options are as follows:
 ...
 -p PACKAGE    The package to generate .pyi files for [fvtk]
...
are put inside the existing fvtk package).  This is equivalent
to the following:

    python -m fvtk.generate_pyi -p fvtk

To put the pyi files somewhere else, perhaps with a different suffix:

    python -m fvtk.generate_pyi -o /path/to/fvtk -e .pyi

To generate pyi files for just one or two modules:

    python -m fvtk.generate_pyi -p fvtk vtkCommonCore vtkCommonDataModel

To generate pyi files for your own modules in your own package:

    python -m fvtk.generate_pyi -p mypackage mymodule [mymodule2 ...]

"""

from fvtk.vtkCommonCore import vtkObjectBase, vtkSOADataArrayTemplate
...
def isnamespace(m):
    ...
    # until fvtk.vtkCommonCore.namespace is directly accessible
    return (str(type(m)) == "<class 'fvtk.vtkCommonCore.namespace'>")
...
def namespace_pyi(c, mod):
    ...
    """
    base = "namespace"
    if mod.__name__ != 'fvtk.vtkCommonCore':
        base = 'fvtk.vtkCommonCore.' + base
...
def module_pyi(mod, output):
    ...
    depends = set(['fvtk.vtkCommonCore'])
...
def main(argv=sys.argv):
    ...
    parser.add_argument('-p', '--package', type=str, default="fvtk",
                        help="Package name [fvtk].")
